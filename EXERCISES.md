...
> The route just returns a (dummy) response for now. However, it requires a valid request (body) to pass a very simplified validation. An example of a valid request can be found in `recources/request.json`
...
Test the API running on AWS by invoking the Load Balanced Web Service URL and the aforementioned API endpoints  (replace `AWS_LB_URL` with your Load Balanced Web Service URL).
    ...
    curl -X POST http://<AWS_LB_URL>/content-request -H "Content-Type: application/json" -d @resources/request.json
...
*   Examine `src/routes.js` and `src/ddb.js`. Currently, a dummy id will be returned since we don't have established connection to DyanomDB, but that will do for now - we will steup DynamoDB later.
...
*   In a second terminal window, invoke the API server:
        ...
        curl -X POST http://localhost/content-request
...
*   Invoke your updated API server (replace `AWS_LB_URL` with your Load Balanced Web Service URL):
        ...
        curl -X POST http://tms-te-Publi-dVSbvJidM3Xw-643285271.eu-north-1.elb.amazonaws.com/content-request -H "Content-Type: application/json" -d @test-data/request.json
...
*   In the `services/content` folder, use the `fault` Express middleware defined in `src/middleware.js` in the Express application created in `src/index.js`.

    The `fault` middleware intercepts call to an `/503` endpoint and injects 503 responses for 3 minutes. 


*   In the `services/api` folder, add a route `/injectfault` in `src/routes.js` that invokes the _TMS Content_ service's `/503` endpoint:
...
*   Implement the healthcheck endpoint `/healthz` the same way as you did for the 
    `api service`, but also take the injected fault into consideration, as below:

*   In the `manifest` file for the `content` service add a healthcheck section as below. 
    Note the difference compared to what we did for the `api service`.

              healthcheck:
                command: ["CMD-SHELL", "wget --spider http://localhost/healthz || exit 1"]
                interval: 5s
                retries: 2
                timeout: 5s
...
*   Send a number of requests:
        ...
        // ubuntu
        siege -r 5 -c 2 "http://<AWS_LB_URL>/content-request POST"
        

        // macOS.
        ab -n 10 -m POST http://<AWS_LB_URL>/content-request
...
To observe autoscaling, send a large number of requests to _TMS API_:
    ...
    // ubuntu
    siege -r 1000 -c 1 "http://<AWS_LB_URL>/content-request POST"
    
    // macOS.
    ab -n 1000 -m POST http://<AWS_LB_URL>/content-request
